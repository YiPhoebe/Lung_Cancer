### ✅ main_test_softmga.py (MGA ⭕️ / 마스크 회전 ⭕️)
...
from ResNet18_CBAM_MGA.core.dataset import CTDataset as DatasetClass, extract_label, load_bbox_dict
...
# 설정
CFG.use_mga = True
CFG.use_rotate_mask = True
CFG.use_tta = True  # TTA 사용 여부 (True면 evaluate_with_tta 사용)

# 모델 로드
CFG.model_save_name = "r18_cbam_mga_sc_softmga_0520_2238.pth"
...
if CFG.use_tta:
    ...
    y_true, y_probs, y_pred = evaluate_with_tta(model, DatasetClass, test_files, test_labels, bbox_dict)
else:
    print("✅ 일반 테스트 수행 중...")
    y_true, y_probs, y_pred = evaluate_with_tta(model, test_dataset, test_loader)
